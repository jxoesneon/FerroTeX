...
*   **`ferrotex-syntax`**: This is the foundation, responsible for parsing LaTeX source code. It uses the `rowan` parser and is designed for *lossless* parsing, ensuring that source fidelity (including trivia) is preserved.
...
*   **Reproducibility**: Always check and maintain the integrity of `ferrotex.lock`. Changes to *any* input file or dependency require updating this lock file to maintain hermetic builds.
