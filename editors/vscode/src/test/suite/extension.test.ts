...
suite("Regression Suite v2 (Comprehensive)", function () {
  ...
  test("Regression: ferrotex.installPackage should not be registered twice (duplicate client bug)", async () => {
    ...
    const installCmds = allCommands.filter((c: string) => c === "ferrotex.installPackage");
  ...
  test("Core: Completion (Standard Latex)", async () => {
    ...
    await waitFor(
      "Server Init",
      async () => (await vscode.languages.getDiagnostics(docUri)).length >= 0,
      5000,
    );
  ...
  test("Core: Definition & Rename (Label/Ref)", async function () {
    ...
    const defs = await vscode.commands.executeCommand<vscode.Location[]>(
      "vscode.executeDefinitionProvider",
      docUri,
      new vscode.Position(1, 15), // inside \ref{sec:intro}
    );
    ...
    const edit = await vscode.commands.executeCommand<vscode.WorkspaceEdit>(
      ...
      docUri,
      new vscode.Position(0, 20), // inside \label{sec:intro}
      "sec:new",
    );
  ...
  test("Feature v0.16.0: Rich Hovers (Citations)", async function () {
    ...
    await waitFor(
      ...
      async () => {
        ...
        const content = (hovers[0].contents[0] as vscode.MarkdownString).value;
...
*/
  ...
  suite("Settings Validation", () => {
    ...
    test("Boolean settings have correct defaults", () => {
      const config = getConfig();
      assert.strictEqual(config.get<boolean>("build.autoBuildOnSave"), true);
      assert.strictEqual(config.get<boolean>("lint.enabled"), true);
      assert.strictEqual(config.get<boolean>("lint.deprecatedCommands"), true);
      assert.strictEqual(config.get<boolean>("completion.enabled"), true);
      assert.strictEqual(config.get<boolean>("format.enabled"), true);
    });
    ...
    test("Enum settings have valid default values", () => {
      ...
      const buildEngine = config.get<string>("build.engine");
      assert.ok(
        ["auto", "tectonic", "latexmk", "pdflatex", "xelatex", "lualatex"].includes(buildEngine!),
      );

      const showOutputPanel = config.get<string>("build.showOutputPanel");
      assert.ok(["always", "onError", "never"].includes(showOutputPanel!));
    });

    test("Number settings have valid ranges", () => {
      const config = getConfig();
      const defaultZoom = config.get<number>("preview.defaultZoom");
      assert.ok(defaultZoom! >= 25 && defaultZoom! <= 500, "defaultZoom should be in range 25-500");

      const indentSize = config.get<number>("format.indentSize");
      assert.ok(indentSize! >= 1 && indentSize! <= 8, "indentSize should be in range 1-8");

      const maxFileSize = config.get<number>("workspace.maxFileSize");
    ...
    test("Critical settings can be updated", async () => {
      ...
      assert.strictEqual(getConfig().get<string>("build.engine"), "tectonic");
...
suite("Hover Functionality Tests", function () {
  ...
  async function getHoverAt(line: number, character: number): Promise<vscode.Hover[]> {
  ...
  test("Graphics: \\includegraphics should show hover", async () => {
    ...
    const text = extractHoverText(hovers);
    assert.ok(
      text.includes("image") || text.includes("graphicx"),
      "Should mention graphics package",
    );
  });
  ...
  test("Package commands show package name", async () => {
    const packageCommands = [
      { line: 40, char: 1, pkg: "graphicx" }, // \includegraphics
      { line: 44, char: 1, pkg: "xcolor" }, // \textcolor
  ...
  test("Regression: \\section should NOT show 'Custom environment'", async () => {
    ...
    assert.ok(
      !text.includes("Custom environment"),
      "Should NOT revert to document environment hover",
    );
  });
