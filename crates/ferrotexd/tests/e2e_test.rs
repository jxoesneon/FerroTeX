...
async fn setup_server() -> (BufReader<ReadHalf<DuplexStream>>, WriteHalf<DuplexStream>) {
    ...
    let (service, socket) = LspService::new(|client| ferrotexd::Backend {
        ...
        package_manager: std::sync::Arc::new(std::sync::Mutex::new(
                ...
                if params["uri"].as_str() == Some(tex_uri.as_str()) {
                    ...
                    if diags.iter().any(|d| {
                        d["message"]
                            .as_str()
                            .unwrap()
                            .contains("Label `foo' multiply defined")
                    }) {
...
async fn test_syntax_diagnostics_flow() -> anyhow::Result<()> {
    ...
    for _ in 0..10 {
        ...
        if msg["method"] == "textDocument/publishDiagnostics"
            && msg["params"]["uri"] == doc_uri.as_str()
            && msg["params"]["diagnostics"]
                .as_array()
                .unwrap()
                .iter()
                .any(|d| d["message"].as_str().unwrap().contains("Expected '}'"))
        {
            found = true;
            break;
        }
